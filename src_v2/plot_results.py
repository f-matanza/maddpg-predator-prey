...
def main():
    ...
    parser.add_argument(
        "--maddpg-csv",
        type    = Path,
        default = Path(config.RESULTS_DIR) / f"maddpg_slow_run_{config.RUN_MARKER}_rewards.csv",
        help    = "CSV log from MADDPG training",
    )
    parser.add_argument(
        "--iddpg-csv",
        type    = Path,
        default = Path(config.RESULTS_DIR) / f"iddpg_slow_run_{config.RUN_MARKER}_rewards.csv",
        help    = "CSV log from Independent DDPG training",
    )
    parser.add_argument(
        "--output",
        type    = Path,
        default = Path(config.RESULTS_DIR) / f"reward_comparison_{config.RUN_MARKER}.png",

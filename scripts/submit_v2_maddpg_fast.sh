#!/bin/bash
#SBATCH --job-name=maddpg_v2_fast
#SBATCH --gpus=1
...
RUN_NAME="${RUN_NAME:-src_v2_maddpg_fast_${JOB_TOKEN}}"
...
srun --cpu-bind=cores "$PYTHON" -u -B -m src_v2.train_fast --algorithm maddpg --run-name "$RUN_NAME"

...
RUN_NAME="${RUN_NAME:-src_v2_iddpg_fast_${JOB_TOKEN}}"
...
srun --cpu-bind=cores "$PYTHON" -u -B -m src_v2.train_fast --algorithm iddpg --run-name "$RUN_NAME"

echo "Training finished: $(date)"
echo "==============================="
echo "Evaluation started: $(date)"

srun --cpu-bind=cores "$PYTHON" -u -B -m src_v2.evaluate --algorithm iddpg --run-name "$RUN_NAME" --tag "$EVAL_TAG" --num-episodes "$EVAL_EPISODES" --fps "$EVAL_FPS"

...
def record_gif(controller, output_path, num_episodes=1, fps=15):
    ...
    for _ in range(num_episodes):
        obs, _ = env.reset()
        while env.agents:
            # --- RENDER PATCH (Visual Fix) ---
            # Push agents out of obstacles so they don't clip in the GIF
            world = env.unwrapped.world
            for agent in world.agents:
                for lm in world.landmarks:
                    dist = np.linalg.norm(agent.state.p_pos - lm.state.p_pos)
                    min_dist = agent.size + lm.size
                    if dist < min_dist and dist > 0.001:
                        # Agent is clipping inside the obstacle -> push it to the exact edge
                        direction = (agent.state.p_pos - lm.state.p_pos) / dist
                        agent.state.p_pos = lm.state.p_pos + direction * min_dist
            # ---------------------------------
